...
import { ScrollReveal, Parallax, TiltCard } from "./Parallax";
...
export default function Hero({ onExploreClick, onPlayVideoClick }: HeroProps) {
  ...
  const quailY = useTransform(scrollYProgress, [0, 1], [0, 120]);
  ...
  return (
    <section
      ref={sectionRef}
      id="hero"
      className="relative overflow-hidden bg-background py-12 lg:py-20 flex items-center min-h-[85vh]"
    >
      {/* Organic radial background glow */}
      <Parallax speed={0.15} className="absolute inset-0 pointer-events-none">
        <div className="organic-bg" />
      </Parallax>

      {/* Quail Bird background watermarked overlay with parallax scroll */}
      <motion.div
        style={{ y: quailY }}
      ...
      <div className="mx-auto max-w-7xl px-6 sm:px-8 w-full z-10">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:items-center">
          ...
          <motion.div style={{ y: textY }} className="flex flex-col justify-center lg:col-span-4 z-20">
            ...
            <ScrollReveal delay={0.25} direction="up">
              <h1 className="font-serif text-5xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight text-secondary leading-[1.1] mb-6">
                ...
                <motion.span 
                  ...
                  className="text-primary italic font-normal inline-block"
                >
                  Tomorrow's
                </motion.span> <br />
                Health Today
          ...
          {/* Center Column: Beautiful Circular Frame with Parallax & 3D Tilt */}
          <motion.div style={{ y: circleY }} className="relative flex items-center justify-center lg:col-span-5 h-[320px] sm:h-[480px]">
            ...
            {/* Center image cutout with interactive 3D Tilt */}
            <TiltCard maxTilt={10} scale={1.03} className="z-10 cursor-pointer">
              <div 
                onClick={onPlayVideoClick}
                title="Watch Farm Video"
                className="relative h-[210px] w-[210px] xs:h-[260px] xs:w-[260px] sm:h-[350px] sm:w-[350px] rounded-full overflow-hidden border-4 sm:border-8 border-background shadow-2xl z-10 transition-transform duration-500 group"
              >
                <Image
                  src="/hero.png"
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-black/10 group-hover:bg-black/35 transition-colors duration-300">
                  <motion.div 
                    ...
                    whileTap={{ scale: 0.9 }}
                    className="h-12 w-12 sm:h-16 sm:w-16 rounded-full bg-white/95 text-primary flex items-center justify-center shadow-2xl scale-90 group-hover:scale-100 group-hover:bg-accent group-hover:text-white transition-all duration-300"
                  >
            ...
            {/* Tiny accent gold bubble with parallax drift */}
            <Parallax speed={-0.3} className="absolute -bottom-2 -left-2 sm:bottom-4 sm:left-4 z-20">
              <motion.div 
                whileHover={{ scale: 1.1, rotate: -5 }}
                className="h-16 w-16 sm:h-20 sm:w-20 rounded-full bg-accent flex flex-col items-center justify-center text-center text-white font-bold text-[10px] sm:text-xs leading-none p-1.5 shadow-lg cursor-pointer"
              >
                <span>100%</span>
                <span>Organic</span>
              </motion.div>
